...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(prog=sys.argv[0], description="Simulate one leg.")
    ...
    parser.add_argument(
        ...
        default="clockwisedown",
        dest="legPositionOnMotor",
    )
...
def createScene(rootnode):
    ...
    load.addObject(
        "MechanicalObject",
        template="Rigid3",
        position=[[0, -200, 80, 0.707, -0.707, 0.0, 0.0]],
    )
    load.addObject("UniformMass", totalMass=myparameters.loadMass)
    visual = load.addChild("Visual")
    visual.addObject(
        "MeshSTLLoader",
        filename=getLoadingLocation(
            "../../data/meshes/centerparts/greymass.stl", __file__
        ),
        translation=[10, 0, 0],
        rotation=[0, 90, 0],
    )
    visual.addObject(
        "OglModel", src=visual.MeshSTLLoader.linkpath, color=RGBAColor.grey
    )
    ...
    platform.addObject(
        "MeshSTLLoader",
        filename=getLoadingLocation(
            "../../data/meshes/supports/platform.stl", __file__
        ),
    )
    platform.addObject(
        "OglModel", src=platform.MeshSTLLoader.linkpath, color=[1, 1, 1, 0.1]
    )
    ...
    for i in range(3):
        ...
        drum.addObject(
            "MeshSTLLoader",
            filename=getLoadingLocation(
                "../../data/meshes/legmotorattachbase.stl", __file__
            ),
            rotation=[0, [90, 180, -90][i], 0],
            translation=[[-100, 0, 0], [0, 0, -100], [100, 0, 0]][i],
        )
        drum.addObject(
            "OglModel", src=drum.MeshSTLLoader.linkpath, color=[1, 1, 1, 0.1]
        )
    ...
    markers.addObject(
        "BarycentricMapping" if args.legModel == "tetra" else "SkinningMapping"
    )
    ...
    if args.connection:
        ...
        try:
            ...
            rootnode.addObject(
                MotorController(
                    [motor.JointActuator.displacement, None, None, None],
