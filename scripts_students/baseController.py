...
class BaseController(Sofa.Core.Controller):
    ...
    def filter(self, signal, signalFilter, cutoffFreq=1., samplingFreq=60.):
        ...
        Args:
            ...
            signalFilter (np.ndarray): The previous filtered signal.
            cutoffFreq (float): The cutoff frequency of the filter.
            samplingFreq (float): The sampling frequency of the signal.
        """
        # === Step 1: Apply a low-pass filter to the signal ===
        signalFilter = None # <<< to be completed
        return signalFilter
