...
class ClosedLoopController(BaseController):
    def __init__(
        ...
        super().__init__(
            leg, motor, markers, load, motorInit, motorMin, motorMax, cutoffFreq
        )

        # add mechanical object for reference
        self.refMo = self.guiNode.addObject(
            "MechanicalObject",
            name="refMo",
            template="Vec3d",
            position=[[0, 0, 0]],
    ...
    def execute_control_at_camera_frame(self):
        ...
        if self.use_observer:
            ...
            measureNoised = self.measurePrev + np.random.normal(
                0, self.guiNode.noise.value, self.markersPos.shape
            )
            self.observerState = (
                self.A @ self.observerState
                + self.B @ cmd
                + self.L @ (measureNoised - self.observerOutput)
            )
        ...
        if self.guiNode.active.value:
            ...
            self.refMo.position.value = np.array(
                [
                    [
                        self.initRefMo[0],
                        markersPos[1],
                        self.initRefMo[2] + self.reference[0, 0],
                    ]
                ]
            )
        ...
        if self.guiNode.controlMode.value == ControlMode["State Feedback"]:
            ...
            desiredMotorPos = (
                self.G @ self.reference - self.K @ state4Control
            ).flatten()
    ...
    def setup_additional_gui(self, use_observer):
        # Specific gui setup
        if use_observer:
            self.guiNode.addData(name="noise", type="float", value=0.0)
        self.guiNode.addData(name="reference", type="float", value=0.0)
        self.guiNode.addData(name="output", type="float", value=0.0)
        self.guiNode.addData(
            name="controlMode", type="bool", value=ControlMode["Open Loop"]
        )
        MyGui.MyRobotWindow.addSettingInGroup(
            "Reference (mm)", self.guiNode.reference, -50, 50, "Control Law"
        )
        if use_observer:
            MyGui.MyRobotWindow.addSettingInGroup(
                "Observer Noise (mm)", self.guiNode.noise, 0, 3, "Control Law"
            )
        MyGui.MyRobotWindow.addSettingInGroup(
            "Control Mode", self.guiNode.controlMode, 0, 1, "Buttons"
        )
    ...
    def record_data(self):
        ...
        if self.record_counter == self.record_max:
            self.guiNode.record.value = False
            self.record_counter = 0
    ...
    def initialize_simulation(self):
        ...
        self.refMo.position.value = np.array(
            [[markerPos[0] - 10, markerPos[1], markerPos[2]]]
        )
    ...
    def save(self):
        ...
        np.savez(
            ...
            legVel=np.array(self.legVelList).reshape(
                len(self.legVelList), self.legVelList[0].shape[0]
            ),
            legPos=np.array(self.legPosList).reshape(
                len(self.legPosList), self.legPosList[0].shape[0]
            ),
            markersPos=np.array(self.markersPosList).reshape(
                len(self.markersPosList), self.markersPosList[0].shape[0]
            ),
            command=np.array(self.commandList).reshape(
                len(self.commandList), self.commandList[0].shape[0]
            ),
            motorPos=np.array(self.motorPosList).reshape(
                len(self.motorPosList), self.motorPosList[0].shape[0]
            ),
            observerState=np.array(self.observerStateList).reshape(
                len(self.observerStateList), self.observerStateList[0].shape[0]
            ),
            observerOutput=np.array(self.observerOutputList).reshape(
                len(self.observerOutputList), self.observerOutputList[0].shape[0]
            ),
            commandMode=np.array(self.commandModeList).reshape(
                len(self.commandModeList), 1
            ),
