...
def get_parser_args():
    parser = argparse.ArgumentParser(
        description="Identify a reduced state-space model."
    )
    parser.add_argument(
        "--order", type=int, default=6, help="Reduction order used to build the model"
    )
    try:
...
def perform_identification():
    ...
    if not os.path.exists(openloop_path):
        raise FileNotFoundError(
            f"Data file not found at: {openloop_path}. Please run the open-loop simulation first."
        )
    ...
    if not os.path.exists(reduction_file):
        raise FileNotFoundError(
            f"Reduction file not found at: {reduction_file}. Please run the reduction script first."
        )
    ...
    # Identify the model
    A, B, C = identify_model(
        reduced_states, inputs, outputs, reduced_positions, args.order
    )
    ...
    # Save model
    np.savez(
        os.path.join(data_path, f"model_order{args.order}.npz"),
        stateMatrix=A,
        inputMatrix=B,
        outputMatrix=C,
    )
