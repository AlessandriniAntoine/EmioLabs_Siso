...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(prog=sys.argv[0], description="Simulate two legs.")
...
class ClosedLoopLinearApp:
    def __init__(
        ...
        for i, item in enumerate(info):
            self.labelInfo[item] = tk.Label(
                infoFrame, text=f"{item}: {infoValue[i]}", font=("Arial", 12)
            )
        ...
        self.sliderMotor = tk.Scale(
            motorSliderFrame,
            from_=-100,
            to=100,
            orient="horizontal",
            command=self.sliderMotorAction,
        )
        ...
        self.sliderRef = tk.Scale(
            refSliderFrame,
            from_=-100,
            to=100,
            orient="horizontal",
            command=self.sliderRefAction,
        )
        ...
        self.startButton = tk.Button(
            buttonFrame, text="Start", command=self.startAction
        )
        self.startButton.pack(side="left", padx=3)

        self.controlModeButton = tk.Button(
            buttonFrame, text="Control Mode", command=self.controlModeAction
        )
        self.controlModeButton.pack(side="right", padx=3)

        self.activeButton = tk.Button(
            buttonFrame, text="Active", command=self.activeAction
        )
        self.activeButton.pack(side="right", padx=3)

        self.recordingButton = tk.Button(
            buttonFrame, text="Record", command=self.recordAction
        )
    ...
    def recordAction(self):
        with self.sharedRecord.get_lock():
            ...
            if self.sharedRecord.value:
                self.active = True
                if self.sharedControlMode.value == ControlMode["Open Loop"]:
                    self.sharedMotorPos[0] = (
                        self.sliderMotor.get() * np.pi / 2 * 1 / 100
                    )
                else:
                    self.sharedRefPos[0] = self.sliderRef.get()
                print("Recording started...")
...
#############################################################################
def filterFirstOder(signal, signalFiltered, cutoffFreq=1.0, samplingFreq=60.0):
...
def processController(
    ...
    maxRecord = 400
    start = False
    cutoffFreqMeasure = 60.0  # Hz
    cutoffFreqMotor = float(args.motorCutoffFreq)  # Hz
    samplingFreq = 60.0  # Hz
    ...
    if not os.path.exists(modelPath):
        raise FileNotFoundError(
            f"Model data file {modelPath} does not exist. Please run the identification script first."
        )
    if not os.path.exists(controlPath):
        raise FileNotFoundError(
            f"Closed loop data file {controlPath} does not exist. Please run the controller script first."
        )
    if not os.path.exists(observerPath):
        raise FileNotFoundError(
            f"Observer data file {observerPath} does not exist. Please run the observer script first."
        )
    ...
    while True:
        ...
        measureFiltered = filterFirstOder(
            measure,
            measureFiltered,
            cutoffFreq=cutoffFreqMeasure,
        ...
        if start:
            ...
            output = (markersPos.reshape(-1, 1))[
                [x for i in range(nbMarkers) for x in [3 * i + 1, 3 * i + 2]]
            ]
            outputPrev = (markersPosPrev.reshape(-1, 1))[
                [x for i in range(nbMarkers) for x in [3 * i + 1, 3 * i + 2]]
            ]
            ...
            observerState = (
                A @ observerState + B @ cmd + L @ (outputPrev - observerOutput)
            )
            ...
            motorPos = filterFirstOder(
                command, motorPos, cutoffFreq=cutoffFreqMotor, samplingFreq=samplingFreq
            )
        ...
        if sharedSave.value:
            print("Data saved...")
            np.savez(
                os.path.join(data_path, "hardware_closedLoop.npz"),
...
def main():
    ...
    # shared variables
    trackerPos = multiprocessing.Array("d", 3 * nbMarkers * [0.0])
